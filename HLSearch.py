# HLSearch.py (改善版 / CPU+GPU統合版)
#
# 探索アルゴリズムは3系統:
#   --exact          : FastSearcher   (DFS+分枝限定法。厳密解。CPU(bigint)のみ、GPU非対応)
#   --beam --cpu     : BeamSearcher   (ビーム法。CPU(bigint)。速度より正当性の基準実装)
#   --beam --gpu(既定): BeamSearcherGPU(ビーム法。CUDA/cupyで高速化。
#                        cupy未導入 or GPUが無い場合は自動的にCPU(numpy)実装にフォールバック)
#
# GPUビーム探索を使うには:
#   pip install cupy-cuda12x   (お使いのCUDAバージョンに合わせて cupy-cudaXXx を選択)
#   NVIDIA GPU + CUDAドライバ
#
# 旧 HLSearch_cuda.py はこのファイルに統合されたため削除して構いません。
import logging
...
from beam_gpu_core import BeamSearcherGPU
...
def main() -> None:
    parser = build_parser(
        description="HLSearch: ビーム法(CPU/GPU)/厳密解法によるシフト探索",
        include_gpu_flag=True,  # --gpu(既定) / --cpu を追加。厳密解(--exact)ではGPU非対応
    )
    ...
    logger.debug(
        ...
        "beam=%s beam_width=%s output_file=%s save_all_best=%s use_gpu=%s",
        rc.depth, rc.limit, rc.target, rc.max_depth, rc.cols,
        rc.use_beam, rc.beam_width, rc.output_file, rc.save_all_best, rc.use_gpu,
    )
    ...
    if rc.use_beam:
        ...
        if rc.use_gpu:
            # cupy/GPUが無ければ BeamSearcherGPU 内部で自動的にCPU(numpy)実装に
            # フォールバックする。強制的にCPU(bigint)の基準実装を使いたい場合は --cpu を指定。
        ...
        # FastSearcher(厳密解法・DFS+分枝限定法)はGPU化していない。
        # 深さ優先探索は枝刈り情報を逐次共有する必要があり、素朴な並列化では
        # 探索量が増えて逆効果になりやすいため。--gpu が明示指定されていても
        # ここではCPU(bigint)の厳密解法にフォールバックし、その旨を警告する。
        if getattr(args, "use_gpu", None) is True:
            logger.warning("厳密解法(--exact)はGPU非対応のため、CPU(FastSearcher)で実行します。")
    ...
    backend = getattr(searcher, "backend", None)
